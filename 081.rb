# In the 5 by 5 matrix below, the minimal path sum from the top left to the bottom right, 
# by only moving to the right and down, is indicated in bold red and is equal to 2427.


# 131 673 234 103 18
# 201 96  342 965 150
# 630 803 746 422 111
# 537 699 497 121 956
# 805 732 524 37  331

# Find the minimal path sum, in matrix.txt (right click and 'Save Link/Target As...'), a 31K 
# text file containing a 80 by 80 matrix, from the top left to the bottom right by only moving 
# right and down.

t1 = Time.now
matrix = ""
dimension = 80

file = File.new("matrix.txt", "r")
while (line = file.gets)
    matrix << line
    matrix << ","
end
file.close

matrix = matrix.split(',').map(&:to_i).each_slice(dimension).to_a

# start with the bottom right and simplify
(dimension - 2).downto(0).each do |i|
  matrix[dimension - 1][i] += matrix[dimension - 1][i + 1]
  matrix[i][dimension - 1] += matrix[i + 1][dimension - 1]
end

(dimension - 2).downto(0).each do |i|
  (dimension - 2).downto(0).each do |j|
    matrix[i][j] += [matrix[i + 1][j], matrix[i][j + 1]].min
  end
end

puts "found #{matrix[0][0]} in #{Time.now - t1}s"
# found 427337 in 0.025162s
